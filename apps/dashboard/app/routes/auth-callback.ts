import { clearOidcLoginTransaction, completeOidcLogin } from "~/lib/oidc.server";
import type { Route } from "./+types/auth-callback";
import { findOrCreateOidcUser } from "~/lib/dashboard-user.server";
import { commitDashboardSession, createDashboardSession } from "~/lib/dashboard-session.server";
...
export async function loader({ request }: Route.LoaderArgs) {
  try {
    ...
    return redirect(login.returnTo, { headers });
  } catch {
